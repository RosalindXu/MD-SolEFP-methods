...
# This program converts .xyz PES scan trajectory, made from *.property.txt from Orca output, to separate .gro files for MM input.
...
awk -v xyz=$xyz -v natoms=$natoms -v nframes=$nframes 'BEGIN{
res[1]="1MeSCN";
res[2]="1MeSCN";
res[3]="1MeSCN";
res[4]="1MeSCN";
...
printf("%s\n", "MM Energy Calculation by Frames");
printf("  %d\n", natoms);
}
